...
test('setId throws for id containing whitespace', function () {
    ...
    expect(fn () => $obj->setId('bad id'))->toThrow(InvalidArgumentException::class);
});

test('setId throws for empty string', function () {
    $obj = new TestId();

    expect(fn () => $obj->setId(''))->toThrow(InvalidArgumentException::class);
});