...
test('setAlpineAttribute with x-data', function () {
    ...
    expect($div->getAttribute('x-data'))
        ->toBe('{ count: 0 }');
});
...
test('setAlpineAttribute with shortcut @click', function () {
    ...
    expect($div->getAttribute('x-on:click'))
        ->toBe('count++');
});
...
test('setAlpineAttribute with shortcut :class', function () {
    ...
    expect($div->getAttribute('x-bind:class'))
        ->toBe('active ? "active" : ""');
});
...
test('setAlpineAttribute with x-show', function () {
    ...
    expect($div->getAttribute('x-show'))
        ->toBe('isVisible');
});
...
test('setAlpineAttribute with x-if on template', function () {
    ...
    expect($template->getAttribute('x-if'))
        ->toBe('condition');
});
...
test('setAlpineAttribute with x-for on template', function () {
    ...
    expect($template->getAttribute('x-for'))
        ->toBe('item in items');
});

test('setAlpineAttribute with x-if on non-template throws exception', function () {
    $div = Division::create($this->document);
    expect(fn () => $div->setAlpineAttribute('if', 'condition'))
        ->toThrow(\InvalidArgumentException::class, 'Alpine directive \'x-if\' is only allowed on <template> elements');
});

test('setAlpineAttribute with x-for on non-template throws exception', function () {
    $div = Division::create($this->document);
    expect(fn () => $div->setAlpineAttribute('for', 'item in items'))
        ->toThrow(
            \InvalidArgumentException::class,
            'Alpine directive \'x-for\' is only allowed on <template> elements'
        );
});
...
test('getAlpineAttribute', function () {
    $div = Division::create($this->document);
    $div->setAlpineAttribute('x-data', '{ count: 0 }');
    expect($div->getAlpineAttribute('x-data'))
        ->toBe('{ count: 0 }');
});
...
test('getAlpineAttributes', function () {
    ...
    expect($attributes)
        ->toHaveKey('x-data', '{ count: 0 }');
    expect($attributes)
        ->toHaveKey('@click', 'count++');
});
...
test('__toString renders shorthand attributes', function () {
    ...
    $html = (string) $div;
    expect($html)
        ->toContain('@click="count++"');
    expect($html)
        ->toContain(':class="active ? &quot;active&quot; : &quot;&quot;"');
    expect($html)
        ->toContain('x-data="{ count: 0 }"');
    expect($html)
        ->not()
