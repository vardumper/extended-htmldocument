...
test('render composed element with parent of', function () {
    ...
    expect($result)->toBeString();
    expect($result)->toContain('THIS FILE IS AUTOGENERATED');
...
test('get global attribute traits', function () {
    ...
    expect($traits)->toHaveKey('className');
    expect($traits)->toHaveKey('id');
});
...
test('trait to property name', function () {
    ...
    expect($method->invoke($this->generator, 'ClassTrait'))->toBe('className');
    expect($method->invoke($this->generator, 'IdTrait'))->toBe('id');
    expect($method->invoke($this->generator, 'HrefTrait'))->toBe('href');
});
...
test('build composed typescript', function () {
    ...
    expect($result)->toContain('THIS FILE IS AUTOGENERATED');
    expect($result)->toContain('Composed Example');
    expect($result)->toContain('import { Anchor }');
});
...
test('determine level', function () {
    ...
    expect($method->invoke($this->generator, 'Html\\Element\\Block\\Div'))->toBe('block');
    expect($method->invoke($this->generator, 'Html\\Element\\Inline\\Span'))->toBe('inline');
    expect($method->invoke($this->generator, 'Html\\Element\\Void\\Img'))->toBe('void');
    expect($method->invoke($this->generator, 'SomeOtherClass'))->toBe('block');
});