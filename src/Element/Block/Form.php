...
class Form extends BlockElement
{
     ...
     */
    protected ?FormEnctypeEnum $enctype = null;

    /**
     * @example get
     */
    protected ?FormMethodEnum $method = null;
     ...
     * @example _self
     */
    protected ?FormTargetEnum $target = null;
    ...
    public function setEnctype(string|FormEnctypeEnum $enctype): static
    {
        if (\is_string($enctype)) {
            $enctype = FormEnctypeEnum::tryFrom($enctype) ?? throw new InvalidArgumentException(
    ...
    {
        if (\is_string($method)) {
            $method = FormMethodEnum::tryFrom($method) ?? throw new InvalidArgumentException(
    ...
    {
        if (\is_string($target)) {
            $target = FormTargetEnum::tryFrom($target) ?? throw new InvalidArgumentException(
