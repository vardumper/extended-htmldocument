...
class NextJSGenerator implements TemplateGeneratorInterface
{
    ...
    private function renderDocumentFromTemplates(HTMLDocumentDelegatorInterface $document): string
    {
        ...
        foreach ($root->childNodes as $child) {
            $rendered = $this->renderNodeUsingNextJSTemplates(
                $child,
                $imports,
                2,
                $parameters,
        ...
        $componentName = $this->getDocumentComponentName();
        $paramsTypeName = $componentName . 'Params';

        $tsx = "/**\n";
        $tsx .= " * THIS FILE IS AUTOGENERATED. DO NOT EDIT IT.\n";
        $tsx .= " *\n";
        $tsx .= ' * @generated ' . date('F j, Y H:i:s') . "\n";
        $tsx .= " * @description Generated document using pre-made @typesafe-html5/react templates\n";
        $tsx .= " */\n\n";
        $tsx .= "import React from 'react';\n";

        if (! empty($imports)) {
            $tsx .= 'import { ' . implode(', ', $imports) . " } from '@typesafe-html5/react';\n\n";
        } else {
            $tsx .= "\n";
        }

        $tsx .= "export interface {$paramsTypeName} {\n";
        foreach ($parameters as $name => $defaultValue) {
            $tsx .= "  {$name}?: string;\n";
        }
        $tsx .= "}\n\n";

        $tsx .= "export const {$componentName}: React.FC<{$paramsTypeName}> = (params) => {\n";
        $tsx .= "  const defaults: Required<{$paramsTypeName}> = {\n";
        foreach ($parameters as $name => $defaultValue) {
            $tsx .= "    {$name}: {$defaultValue},\n";
        }
        $tsx .= "  };\n";
        $tsx .= "  const values = { ...defaults, ...params };\n";
        $tsx .= "  return (\n";
        $tsx .= "    <>\n";

        if ($jsxNodes !== []) {
            $tsx .= implode('', $jsxNodes);
        }
    ...
    private function renderNodeUsingNextJSTemplates(
        \DOM\Node $node,
        array &$imports,
        int $indentLevel,
        array &$parameters,
        ...
        if ($node instanceof \DOM\Text) {
            ...
            $parameterName = $this->registerDocumentParameter(
                'content' . (++$textCounter),
                $this->toTypeScriptStringLiteral($text),
                $parameters
            );

            return $indent . '{values.' . $parameterName . "}\n";
        }
        ...
        if ($tag === 'html' || $tag === 'head' || $tag === 'body') {
            ...
            foreach ($node->childNodes as $child) {
                $output .= $this->renderNodeUsingNextJSTemplates(
                    $child,
                    $imports,
                    $indentLevel,
                    $parameters,
        ...
        $componentName = $this->resolveNextJSComponentNameFromTag($tag);
        $imports[] = $componentName;
        $imports = array_values(array_unique($imports));
        $nodeCounter++;
        $nodeToken = 'node' . $nodeCounter;

        $attributes = $this->buildNextJSAttributeString($node, $parameters, $nodeToken);
        $children = '';
        foreach ($node->childNodes as $child) {
            $children .= $this->renderNodeUsingNextJSTemplates(
                $child,
                $imports,
                $indentLevel + 1,
                $parameters,
        ...
        if (trim($children) === '') {
            $suffix = $attributes === '' ? '' : ' ' . $attributes;
            return $indent . "<{$componentName}{$suffix} />\n";
        }
    ...
    private function buildNextJSAttributeString(\DOM\Element $element, array &$parameters, string $nodeToken): string
    {
        $parts = [];
        $dataSet = [];
        ...
        foreach ($element->attributes as $attribute) {
            ...
            if (str_starts_with($name, 'data-')) {
                $dataSet[substr($name, 5)] = $value;
                continue;
            }

            $propName = $this->normalizeNextJSPropName($name);
            if ($value === '') {
                $parts[] = "{$propName}={true}";
                continue;
            }
            ...
            $parameterName = $this->registerDocumentParameter(
                $nodeToken . $this->toParameterToken($name),
                $this->toTypeScriptStringLiteral($value),
                $parameters
            );

            $parts[] = "{$propName}={values.{$parameterName}}";
        }
        ...
        if ($dataSet !== []) {
            ...
            foreach ($dataSet as $key => $value) {
                $parameterName = $this->registerDocumentParameter(
                    $nodeToken . $this->toParameterToken($key),
                    $this->toTypeScriptStringLiteral($value),
                    $parameters
                );
                $entries[] = $this->toObjectKeyLiteral($key) . ': values.' . $parameterName;
            }
            $parts[] = 'dataSet={{ ' . implode(', ', $entries) . ' }}';
        }

        return implode(' ', $parts);
    }

    private function normalizeNextJSPropName(string $attributeName): string
    {
        if ($attributeName === 'class') {
            return 'className';
        }

        $camel = lcfirst(str_replace(' ', '', ucwords(str_replace(['-', '_', ':'], ' ', $attributeName))));

        if (in_array($camel, self::JS_RESERVED_WORDS, true)) {
            return 'html' . ucfirst($camel);
        }

        return $camel;
    }

    private function resolveNextJSComponentNameFromTag(string $tag): string
    {
        $className = $this->getElementByQualifiedName($tag);
        if ($className !== null) {
            $ref = new ReflectionClass($className);
            $elementName = $ref->hasConstant('QUALIFIED_NAME')
                ? (string) $ref->getConstant('QUALIFIED_NAME')
    ...
    private function getDocumentComponentName(): string
    {
        $handle = trim($this->componentHandle);
        if ($handle === '') {
            return 'RenderDocument';
        }

        $parts = preg_split('/[^a-zA-Z0-9]+/', strtolower($handle)) ?: [];
    ...
    private function registerDocumentParameter(string $name, string $defaultValue, array &$parameters): string
    {
        $parameterName = $this->normalizeParameterName($name);
        $baseName = $parameterName;
        $suffix = 2;

        while (array_key_exists($parameterName, $parameters)) {
    ...
    {
        $normalized = preg_replace('/[^a-zA-Z0-9_]/', '_', $name) ?? 'param';
        if ($normalized === '' || ctype_digit($normalized[0])) {
            return 'param_' . $normalized;
        }

        return $normalized;
    }

    private function toParameterToken(string $value): string
    {
        $token = preg_replace('/[^a-zA-Z0-9]+/', ' ', $value) ?? $value;

        return str_replace(' ', '', ucwords(trim($token)));
    }

    private function toObjectKeyLiteral(string $value): string
    {
        if (preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/', $value) === 1) {
            return $value;
        }

        return $this->toTypeScriptStringLiteral($value);
    }

    private function toTypeScriptStringLiteral(string $value): string
    {
        return "'" . str_replace(
            ['\\\\', "'", "\n", "\r", "\t"],
            ['\\\\\\\\', "\\'", '\\n', '\\r', '\\t'],
