...
class TypeScriptGenerator implements TemplateGeneratorInterface
{
    ...
    private function renderDocumentFromTemplates(HTMLDocumentDelegatorInterface $document): string
    {
        ...
        $lines = [];
        $parameters = [];
        $apiBaseName = $this->getDocumentApiBaseName();
        $paramsTypeName = $apiBaseName . 'Params';

        $root = $document->delegated->body ?? $document->delegated;
        foreach ($root->childNodes as $child) {
            $nodeRef = $this->renderNodeUsingTypeScriptTemplates(
                $child,
                $imports,
                $lines,
                $counter,
            ...
            if ($nodeRef !== null) {
                $lines[] = "  root.append({$nodeRef});";
            }
        }

        sort($imports, \SORT_STRING);
        $importLine = empty($imports)
            ? ''
            : 'import { ' . implode(', ', $imports) . " } from '@typesafe-html5/typescript';\n\n";

        $ts = "/**\n";
        $ts .= " * THIS FILE IS AUTOGENERATED. DO NOT EDIT IT.\n";
        $ts .= " *\n";
        $ts .= ' * @generated ' . date('F j, Y H:i:s') . "\n";
        $ts .= " * @description Generated component using pre-made @typesafe-html5/typescript templates\n";
        $ts .= " */\n\n";
        $ts .= $importLine;
        $ts .= "export interface {$paramsTypeName} {\n";
        foreach ($parameters as $name => $defaultValue) {
            $ts .= "  {$name}?: string;\n";
        }
        $ts .= "}\n\n";

        $ts .= "export function {$apiBaseName}(params: {$paramsTypeName} = {}): DocumentFragment {\n";
        $ts .= "  const defaults: Required<{$paramsTypeName}> = {\n";
        foreach ($parameters as $name => $defaultValue) {
            $ts .= "    {$name}: {$defaultValue},\n";
        }
        $ts .= "  };\n";
        $ts .= "  const values = { ...defaults, ...params };\n";
        $ts .= "  const root = document.createDocumentFragment();\n";
    ...
    private function renderNodeUsingTypeScriptTemplates(
        \DOM\Node $node,
        array &$imports,
        array &$lines,
        int &$counter,
        ...
        if ($tag === 'html' || $tag === 'head' || $tag === 'body') {
            ...
            foreach ($node->childNodes as $child) {
                $childRef = $this->renderNodeUsingTypeScriptTemplates(
                    $child,
                    $imports,
                    $lines,
                    $counter,
        ...
        $className = $this->resolveTypeScriptClassNameFromTag($tag);
        $imports[] = $className;
        $imports = array_values(array_unique($imports));

        $counter++;
        $varName = 'node' . $counter;
        $lines[] = "  const {$varName} = new {$className}().getElement();";

        foreach ($node->attributes as $attribute) {
            $parameterName = $this->registerDocumentParameter(
                $varName . $this->toParameterToken($attribute->name),
                $this->toTypeScriptStringLiteral($attribute->value),
                $parameters
            );
            $attrName = $this->toTypeScriptStringLiteral($attribute->name);
            $lines[] = "  {$varName}.setAttribute({$attrName}, values.{$parameterName});";
        }
    ...
    private function resolveTypeScriptClassNameFromTag(string $tag): string
    {
    ...
    private function getDocumentApiBaseName(): string
    {
        ...
        if ($camel === '' || ctype_digit($camel[0])) {
            $camel = 'component' . ucfirst($camel);
        }
