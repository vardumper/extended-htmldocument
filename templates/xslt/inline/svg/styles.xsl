...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="item[@type='svg']">
        <xsl:element name="svg">
            ...
            <xsl:if test="fragment[@name='xmlns']">
                <xsl:attribute name="xmlns">
                    <xsl:value-of select="fragment[@name='xmlns']"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="fragment[@name='viewBox']">
                <xsl:attribute name="view-box">
                    <xsl:value-of select="fragment[@name='viewBox']"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="fragment[@name='stroke']">
                <xsl:attribute name="stroke">
                    <xsl:value-of select="fragment[@name='stroke']"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="fragment[@name='fill']">
                <xsl:attribute name="fill">
                    <xsl:value-of select="fragment[@name='fill']"/>
                </xsl:attribute>
            </xsl:if>
            ...
            <xsl:apply-templates select="item[@type='a' or @type='script' or @type='style' or @type='svg' or @type='title']"/>
        </xsl:element>
    </xsl:template>
