...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="item[@type='tr']">
        <xsl:element name="tr">
            <xsl:if test="@id">
            ...
            <xsl:if test="fragment[@name='align'] and contains('|left|center|right|justify|char|', concat('|', fragment[@name='align'], '|'))">
                <xsl:attribute name="align">
                    <xsl:value-of select="fragment[@name='align']"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="fragment[@name='bgcolor']">
                <xsl:attribute name="bgcolor">
                    <xsl:value-of select="fragment[@name='bgcolor']"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="fragment[@name='char']">
                <xsl:attribute name="char">
                    <xsl:value-of select="fragment[@name='char']"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="fragment[@name='charoff']">
                <xsl:attribute name="charoff">
                    <xsl:value-of select="fragment[@name='charoff']"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="fragment[@name='valign'] and contains('|baseline|bottom|middle|top|', concat('|', fragment[@name='valign'], '|'))">
                <xsl:attribute name="valign">
                    <xsl:value-of select="fragment[@name='valign']"/>
                </xsl:attribute>
            </xsl:if>
            ...
            <xsl:if test="fragment[@name='ariaSelected'] and contains('|false|true|undefined|', concat('|', fragment[@name='ariaSelected'], '|'))">
                <xsl:attribute name="aria-selected">
                    <xsl:value-of select="fragment[@name='ariaSelected']"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="fragment[@name='ariaPosinset']">
                <xsl:attribute name="aria-posinset">
                    <xsl:value-of select="fragment[@name='ariaPosinset']"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="fragment[@name='ariaSetsize']">
                <xsl:attribute name="aria-setsize">
                    <xsl:value-of select="fragment[@name='ariaSetsize']"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="fragment[@name='ariaLevel']">
                <xsl:attribute name="aria-level">
                    <xsl:value-of select="fragment[@name='ariaLevel']"/>
                </xsl:attribute>
            </xsl:if>
            ...
            <xsl:if test="fragment[@name='ariaSort'] and contains('|none|ascending|descending|other|', concat('|', fragment[@name='ariaSort'], '|'))">
            ...
            <xsl:apply-templates select="item[@type='td' or @type='th']"/>
        </xsl:element>
    </xsl:template>
